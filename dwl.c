...
arrange(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link) {
		...
		// ⬇️ Apply top offset to all visible, non-fullscreen clients
		if (VISIBLEON(c, m) && !c->isfullscreen) {
			wlr_scene_node_set_position(&c->scene->node,
				c->geom.x, c->geom.y + WINDOW_TOP_GAP);
		}
	}
...
drawbar(Monitor *m)
{
	...
	/* Draw status text (slstatus) on selected monitor */
	if (m == selmon) {
		drwl_setscheme(m->drw, colors[SchemeNorm]);
		tw = TEXTW(m, stext) - m->lrpad + 2;
		drwl_text(m->drw, m->b.width - tw, 0, tw, m->b.height, 0, stext, 0);
	}
	/* Collect tag occupancy and urgency */
	wl_list_for_each(c, &clients, link) {
		if (c->mon != m)
			continue;
...
*/
	...
	for (i = 0; i < LENGTH(tags); i++) {
		/* Skip tags that are empty and not selected */
		...
		// fill space between last tag and status
		// same as: drw_text(drw, x, ..., " ", ...) in dwm
	}

	/* Fill the space where title used to be */
	w = m->b.width - tw - x;
	if (w > 0) {
		drwl_setscheme(m->drw, colors[SchemeNorm]);  // make sure SchemeNorm is #222222 bg
		drwl_rect(m->drw, x, 0, w, m->b.height, 1, 1);
	}

	/* Final buffer commit to scene graph */
